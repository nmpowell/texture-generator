...
| Wood, plastic, paper | `normal_strength` | `float` | Wood: an exaggeration factor on the board's *physical* slopes — 1.0 renders the true slope the millimetre-scale height field describes; draws 1.1–2.2, since a board this flat in true millimetres still needs a little exaggeration to read as relief on a flat texture map. Plastic draws glossy 0.5–1.1, matte 1.5–3.0, textured 1.4–2.6, on an arbitrary (not physical) normalised range. Paper defaults to 1.0 and applies it to the diffuse normal. |
...
| Wood | `light_dir` | Three floats `(x, y, z)` | Defaults to `(-0.5, -0.55, 0.78)`, pointing towards the light. Change it to inspect chatoyance under another lighting direction — the [workflow notebook](../examples/texture_generator_workflow.ipynb) renders one board under six light azimuths side by side ("Wood chatoyance") to show the effect. |
...
| Scale and filtering | Height is carried in real **millimetres**, not normalised, and `height_spacing` (`1/px_per_mm`) makes every normal resolution-independent, so a board's slopes are the same whatever pixel size it is rendered at rather than a per-texel gradient that got steeper as the render grew. Relief sums latewood's proud band (`LATEWOOD_RELIEF_MM`, 30 µm), fine grain streaks (`STREAK_RELIEF_MM`, fbm std ~0.25 → ~8 µm RMS) and broad machining waviness (`WAVINESS_MM`), minus the pore troughs — all three amplitudes are **UNVERIFIED** working figures, not profilometry. The fine-streak fbm's across-grain frequency is no longer clamped to the sampling grid; instead `fbm_at`'s `max_freq` fades out whatever octave the grid cannot carry (the sampling Nyquist), dropping that energy rather than re-authoring it at the grid's own pitch. Sub-pixel pores are carried as **coverage**, not re-drawn full-contrast dashes: a vessel narrower than a texel is drawn at the grid's own width and faded so the mean coverage matches the anatomy (see *Notes and limits* for the fine-vesselled-species caveat this leaves). All three are what the **mip test** (`test_a_small_render_matches_a_downsampled_large_one`) checks: a small direct render and a large render box-downsampled in linear light must agree in mean and contrast — before this change a direct 500px oak render's luminance std (12.99) sat 33% above an equivalent 2000px downsample's (9.77); after, 7.55 vs 7.92 |
| Finish | Finishing is not a tint: raw wood's air/cell-wall interface (n 1.00 → 1.53) scatters light back as a white veil, and a finish index-matches it away. This is a **fitted CIELAB appearance approximation**, applied in CIELAB — oil ΔL\* −8/ΔC\* +8, polyurethane −6/+9, acrylic −4/+4 — and it carries no physical darkening of its own; the finish's actual optics (film build, refractive index, fibre tint) are separate, explicit state, described in **Coat** below |
| Coat | A finish is two separate states. Per board, a **film build** (`film_um`, a `(lo, hi)` microns range per finish) self-levels over the wood's relief — a lift `coat_lift = clip(blur(height, COAT_LEVEL_MM) - height, 0, film_mm)` above the substrate, with an edge-padded blur so the board's far edge is never its own neighbour — and pools a pore no deeper than it is built; the coat gloss's pore break uses the fraction of each vessel's *physical* trough left open above the film, not the texel's area coverage. The **coat normal** (from `height + coat_lift`) drives the *surface* specular lobes only; the diffuse term, cavity and the fibre/ray lobes always use the substrate normal, since light reaching the fibres has already crossed the film. A per-finish **refractive index** (`ior`) refracts the light into that film before the fibre and ray lobes see it. A per-finish **fibre tint** colours the fibre lobe for the light that has passed through the film. Provenance, all **UNVERIFIED**: `film_um` none/oil (0, 0) [a penetrating finish leaves no continuous film]; acrylic (25, 60), polyurethane (50, 110) [trade figures for 2-3 coats]; `ior` 1.0 where there is no film, 1.50 for acrylic/polyurethane [the conventional generic-finish value of rendering practice, not a measured cured-film index]; `fibre_tint` neutral for bare wood, near-neutral for the non-yellowing acrylic, amber for oil and polyurethane [the existing amber, kept because an oil-based finish warms the light it passes, film or not] |
| **Chatoyance** | Aligned fibres reflect anisotropically, so wood's luster shifts with angle. The fibre-tangent field is **the gradient of the same domain warp that bends the rings** — following Liu et al., *Simulating the Structure and Texture of Solid Wood* (ACM TOG 35(6), 2016), where the fibre directions "follow naturally from the distortions", so no second figure system is needed. Shaded with **two lobes**: a sharp, mostly isotropic surface lobe from the finish film (shaded off the **coat** normal above) over a soft anisotropic fibre lobe coloured `sqrt(albedo)` and tinted by the finish. The light reaching that fibre lobe is refracted into the coat first (`fibre_ior`), which moves its peak dip from −15.0° to −9.3° under the default light at n 1.5. Ray fleck rides a **second, crosswise fibre population** mixed into the same lobe by weight (`ray_tangent`/`ray_weight`/`ray_gain`), not a rotation of the grain texture — see **Ray fleck** below. That layering is what gives a polished panel depth. The [workflow notebook](../examples/texture_generator_workflow.ipynb) renders one curly maple board and one quartersawn oak board under six light azimuths side by side ("Wood chatoyance") to show it moving |
...
| Why it *flashes* | Rays are radial, so on a quartersawn face the ray tissue's fibre direction is perpendicular to the axial grain. The ray tissue is therefore a **second fibre population** with its own axis — the fibre's in-plane angle turned 90°, lying in the face with no dip — and inside a fleck the fibre lobe is a mixture of the two populations weighted by the fleck mask (`ray_tangent`, `ray_weight`, `ray_gain` 1.25), rather than a rotation of the grain axis, which would invent diagonal fibres on the fleck's rim and hand the rays the fibre's dip. That crosswise lobe is what makes a fleck light up and go dark as the light moves. Ray fleck is a chatoyance effect, not a pigment one — the albedo contrast is only ΔL\* 3–8 and the second population does the work |

Not yet modelled, deliberate deferrals recorded in the design triage rather
than omissions: exact Fresnel/Schlick reflectance at the fixed view (measured
as a no-op — `F(wi.h)` differs from `F0 = 0.04` by under 1.3% at the 99.9th
percentile across the `normal_strength` range, so `FINISH_F0` stays the
conventional n 1.5 starting point rather than a per-pixel Fresnel term);
anisotropic GGX + Smith for the finish lobe (the Blinn-Phong/Ward-style blend
is kept — at a fixed +z view the tail difference between the two is only ever
exercised by relief slopes, where it is invisible); Beer-Lambert film
absorption (no film-thickness/absorption field exists; the film's amber cast
already lives in the fitted CIELAB Δb\* and the per-finish fibre tint, and
stacking a Beer term on top of both would double-count it); a full BSSRDF or
volume light-transport model, or integrator tiers generally (this is a
fixed-view flat texture map, not a renderer); and the volumetric 3-D solid-wood
field of Liu et al. that would make flatsawn, quartersawn and end grain all
fall out of one model instead of being separate cases.
...
| Module | Contents |
| --- | --- |
| `core/noise.py` | `gradient_noise`, `fbm`, `ridged` (+ `*_at` variants for arbitrary coordinate arrays); vectorised Perlin-style lattice noise with quintic fade and anisotropic `(freq_x, freq_y)` support. The gradient lattice is sized to the coordinates actually sampled, so tall canvases and warped/ring coordinates never band-repeat; a per-axis `periodic` flag restores unit wrapping where it is wanted (radial metal's θ axis). `fbm_at`'s optional `max_freq` band-limits the sum to the sampling grid's own Nyquist frequency, fading out (not aliasing) whatever octave the grid cannot carry; every octave is still drawn and the amplitude normalisation is unchanged, so dropped energy stays dropped rather than being redistributed onto the octaves that remain |
...
| `core/fields.py` | `normalize01`, `remap`, `smoothstep`, `height_to_normal`, `blur`, `draw_segments` (PIL-rasterised signed line scatter). `height_to_normal`'s `spacing` argument is the physical distance between height samples, so the same surface yields the same normal whatever grid it is sampled on; `1.0` (the default) reproduces the historical per-texel gradient exactly |
...
| `core/shading.py` | `shade(...)` — Lambert diffuse over height-derived normals with a high (slightly sky-directional) ambient floor, Blinn-Phong specular with Ward-style anisotropic lobe; `specular`/`shininess` accept per-pixel `(H, W)` arrays (ring gloss, pass bands, smudges), `spec_tint` tints the highlight towards the albedo for conductors (with Schlick whitening on steep micro-slopes), `specular2`/`shininess2` add a second tight clearcoat lobe, `cavity` darkens local concavities (`cavity_depth` gives the physical recess depth at which that darkening is complete, so one deep feature such as a plank gap does not wash out every shallower recess; without it the darkest recess is always fully dark), and `normalise` (**off by default**, on only for wood) divides the lighting by its own mean so a measured albedo survives the shading pass. `height_spacing` scales every gradient computed inside `shade`, so a board's normals are resolution-independent rather than per-texel. `coat_height` swaps in a second, smoother normal field for the surface lobes only, for a self-levelling film sitting over a rougher substrate. `fibre_ior` refracts the light into that film before the fibre and ray lobes see it, so a coated finish's chatoyance peaks at a different angle than bare wood's. `ray_tangent`/`ray_weight`/`ray_gain` mix a second, crosswise fibre population into the fibre lobe by weight rather than by rotating the tangent field, for wood's ray fleck. `shade_translucent(...)` shades a thin scattering sheet with **two** normals — blurred for diffuse (the subsurface point-spread radius), sharp for sheen — with Oren–Nayar roughness, wrap diffuse, and mean-normalised lighting; `gaussian_blur` is the separable blur both use, wrap-around by default and edge-padded with `mode="edge"` for fields that do not tile |
...
`tests/test_wood_optics.py` covers the wood-optics change specifically: axial
sign invariance, coat/fibre lobe separation, figure surviving a flattened
albedo, the `finish="none"` zero-coverage case, pore pooling under a film,
refraction of the fibre-lobe peak, ray-population isolation, band-limited
noise, linear-light compositing and the mip (small-direct-render-vs-
downsampled-large-one) sweep, each checked against a closed-form oracle or an
exact invariant rather than the implementation's own output. It documents what
a fixed-view, 2-D texture generator cannot validate: reciprocity, the
white-furnace check, Snell across total internal reflection, grazing
bump-vs-displacement parallax, angular hold-out/reference-path convergence,
and Beer-law film-absorption monotonicity, since no thickness/absorption field
exists here for it to hold against.
...
- Wood's sub-pixel vessels are carried as area-honest coverage (see *Scale and
  filtering*), but that trades away a photograph's own per-pixel variance:
  fine-vesselled species (cherry, maple, mahogany), whose vessels are narrower
  than a texel at any sane render size, render their pores as the uniform tone
  the grid can carry, not the Poisson-distributed per-pixel variance a
  photograph at 0.1 mm per pixel would show. That variance is deferred, not
  modelled. Two related limits of the same construction: a sub-pixel vessel is
  drawn at the noise lattice's 2 px minimum width, so in a direct small render
  its peak contrast is about half what a box-downsampled large render shows
  even though the mean darkening matches; and at 512 px and below most boards'
  fine streak layer (0.4–2.5 mm pitch) is sub-pixel and averages away, as it
  does in a downsampled large render. Rendering at 1000 px or more and
  resampling down, the documented workflow, sidesteps both.
- Plank gaps and bevels are in millimetres (`PLANK_GAP_MM` 0.5–1.5,
  `PLANK_GAP_DEPTH_MM` 0.8, `PLANK_BEVEL_MM` 1.0, all UNVERIFIED), so a seam
  is the same groove at every render size; a gap narrower than a texel is
  still one texel wide.
