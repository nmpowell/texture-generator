...
Status: implemented and targeted checks pass. The visual preset remains a procedural prototype. The latest revision removes broad comb ribbons and reduces nucleus concentration, but attached branches still look fan-like under revealing camera angles. No specimen-calibrated appearance claim is made.

## Contract and replay

`materials.galvanised` exposes `GENERATOR_VERSION = "galvanised-2"`, `GALVANISED_PRESETS`, frozen `GalvanisedState`, `build_state`, `sample_points`, `sample_state`, `optical_parameters`, `surface_resource_hashes`, and the legacy float32 RGB `generate` adapter. A seeded state uses PCG64 and exactly one `material_key_from_rng` draw. An explicit key bypasses that draw; an optional seed alongside the key is replay metadata. Topology, weather and fabrication object streams are separately named. Resolution, chunk size, lighting and exposure do not enter topology creation.
...
Doubled-angle branch directions retain their bounded coherence rather than turning vanishing support into a strong arbitrary axis. The sampler combines this descriptor with a weak intrinsic crystal axis (weights 0.85 and 0.15). This is an authoring proxy, not a calibrated microfacet distribution. No colour variation is used to simulate grain reflections.
...
The authoring-4 seed-42, 24 × 18 mm, 192 × 144 diagnostic has height RMS 0.349 µm, a −1.316 to +1.090 µm range and normal XY RMS 0.00230. These are one prototype's measurements, not a fitted industrial envelope. Camera positions near the studio emitter's reflected direction expose the remaining fan-like construction. Warp remains disabled.

Environmental coefficients are fixed periodic broad fields in a weather-only namespace. A smooth moisture threshold leaves bare-zinc gaps; `Cp=1−exp(−k·exposure)` is monotone and `Cw` is a wetness/confinement/white-stain-controlled subset. Visible fractions are `(1−Cp, Cp−Cw, Cw)`. Deposits share these masks and contribute no height where coverage is absent. The fresh zinc intrinsic roughness/anisotropy and substrate are unchanged by weather presets; the visible compatibility maps mix separate deposit optics. Dross and drainage are sparse physical object records with Poisson area counts, compact radius/height/path and gravity, experimental outside the initial sheet presets.
